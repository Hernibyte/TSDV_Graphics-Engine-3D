...
ModelImporter::ModelImporter(Renderer* render) {
    this->render = render;
}

void ModelImporter::LoadModel(std::string const& path) {
...
Mesh ModelImporter::ProcessMesh(aiMesh* mesh, const aiScene* scene) {
    ...
    return Mesh(vertices, indices, textures, *render);
}
...
std::vector<Texture> ModelImporter::LoadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName) {
    ...
    for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
    {
        ...
        {
            Texture texture;
            texture.texture = TextureImporter::ImportFromFile(str.C_Str(), this->directory, false);
